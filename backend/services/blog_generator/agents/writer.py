...
class WriterAgent:
    ...
    def write_section(
        ...
        # 获取 Humanizer 写作避坑指南
        from ..skills.humanizer import skill as humanizer_skill

        humanizer_guide = humanizer_skill.get_writing_guide()
        ...
        prompt = pm.render_writer(
            ...
            humanizer_guide=humanizer_guide,  # 传递指南
        )
    ...
    def enhance_section(
        ...
        prompt = pm.render_writer_enhance(
            original_content=original_content, vague_points=vague_points
        )
    ...
    def run(self, state: Dict[str, Any], max_workers: int = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"开始撰写内容: {len(tasks)} 个章节，使用 {min(max_workers, len(tasks))} 个并行线程"
        )
        ...
        def write_task(task):
            ...
            try:
                ...
                logger.error(
                    f"章节撰写失败 [{task['section_outline'].get('title', '')}]: {e}"
                )
                return {
                    "success": False,
                    "order_idx": task["order_idx"],
